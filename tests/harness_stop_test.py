...
one moment; a flagged moment is handed to the main agent once, at the next
prompt, with its stamp; subagents never take a moment; nothing in the sensor
sends `activate`. No test reaches a server: `server_classify` is substituted.
"""
...
def test_the_next_prompt_hands_the_moment_to_the_main_agent_once():
    with _Env():
        ...
        assert not hx.read_jsonl(hs.moments_path("sess"))[0].get("handed_at")
        ...
        assert hx.read_jsonl(hs.moments_path("sess"))[0].get("handed_at")
