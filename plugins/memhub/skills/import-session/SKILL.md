---
description: Use when the user asks to import, upload, or save a Claude Code, Codex, or Cursor session/conversation/transcript into MemHub (e.g. "import this session into memhub", "save session <id> to memhub", "backfill that conversation into my memory"). Ships the transcript into the user's personal memory via a terminal upload script — any size, no token-by-token re-emit.
...
Import a past Claude Code, Codex, or Cursor session into the user's MemHub
memory on demand. The unified capture script locates the host session,
normalizes it when needed, and ships it to the `import_conversation` MCP tool —
**do NOT call the MCP tool yourself and do NOT read or paste transcript
content**; sessions can exceed a million tokens and the script handles any size
without putting the transcript in model context. This is a terminal operation.
...
1. **The destination is always the user's personal memory.** Sessions are never
   imported into an agent brain: automatic capture sends every session to
   personal memory, and an import that routed one would split it into a second
   conversation. There is no flag to change this. If the user wants something
   team-visible, that belongs in an artifact (`/memhub:save-artifact`).
...
2. Run the import in the terminal — one command, substitute the real values:
   ...
   host. NEVER pass `--conversation-id`. Omitted, Claude uses the session id and
   Codex/Cursor use the same host-prefixed id as automatic capture. That keeps
   the session one conversation and makes re-imports incremental. A fresh id
   would split the session's memory. If nothing new lands, automatic capture
   already did its job; do not work around that with a new id.
...
3. Report back the returned `conversation_id`, `source_platform`, `path` (should
   be `"agentic"`), and `messages_received`. Tell the user:
   - it landed in **their personal memory**;
   - extraction runs in the background (allow several minutes for large
     sessions) — episodes, directives, and the session **gist** land in
