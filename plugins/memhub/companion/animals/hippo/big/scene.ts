// Layer 2 — scene. hippo_band.py's render_canvas: a 22x12 grid of RGB (or
// null for transparent) holding the pond, the hippo, the squid and the
// particles. Nothing here knows about a terminal, a clock or randomness —
...
function drawPond(cv: Canvas, tick: number) {
  POND.forEach((row, r) => {
  ...
  for (const i of [0, 1]) {
    // two slow glints on the water
    const x0 = WATER[0] + 2 + i * 7 + ((Math.floor(tick / 9) + i) % 3)
    for (let x = x0; x < x0 + 2; x++) {
      if (x >= WATER[0] && x <= WATER[1]) cv[HY - 1]![x] = PAL.w!
    }
  }
}
  ...
  if (r >= 0 && r < SPRITE_H) {
    ...
    for (let x = lo - 1; x < hi + 2; x++) {
      if (x >= WATER[0] && x <= WATER[1]) {
        ...
        cv[HY]![x] = edge || (x + Math.floor(tick / 3)) % 3 ? PAL[edge ? 'b' : 'w']! : PAL.D!
      }
    }
  }
}
